    ...
    return (
        <div style={{
            ...
            <div style={{ padding: '10px 14px 0' }}>
                ...
                {hint && <p style={{ margin: '2px 0 0', fontSize: '0.73rem', color: '#9090a0' }}>{hint}</p>}
            </div>
            ...
            <div style={{ padding: '12px 14px 14px' }}>
                ...
                {estado === 'idle' && (
                    <div
                        ...
                        onDrop={handleDrop}
                    >
                        <Upload size={24} color="#00abbf" style={{ marginBottom: '6px' }} />
                        <p style={{ margin: '0 0 4px', fontSize: '0.83rem', fontWeight: 600, color: '#1a1a2e' }}>
                            Arrastra un archivo aquí o <span style={{ color: '#00abbf' }}>selecciona</span>
                        </p>
                ...
                {estado === 'procesando' && (
                    <div style={{ textAlign: 'center', padding: '16px 0' }}>
                        <Loader2 size={22} color="#00abbf" style={{ animation: 'spin 1s linear infinite', marginBottom: '8px' }} />
                        <p style={{ margin: '0 0 4px', fontSize: '0.83rem', fontWeight: 600, color: '#1a1a2e' }}>
                            Extrayendo contenido de {archivo?.name}...
                        </p>
                        {progresoPdf && (
                            <p style={{ margin: 0, fontSize: '0.77rem', color: '#00abbf' }}>{progresoPdf}</p>
                        )}
                    </div>
                )}
                ...
                {estado === 'completado' && resultado && (
                    <div>
                        <div style={{
                            ...
                            <span style={{ fontSize: '1.3rem' }}>{TIPO_ICONOS[resultado.tipo]}</span>
                            <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    <span style={{ fontWeight: 700, fontSize: '0.83rem', color: '#1a1a2e' }}>
                                ...
                                <p style={{ margin: '2px 0 0', fontSize: '0.73rem', color: '#9090a0' }}>
                        ...
                        <div style={{
                            padding: '8px 12px', borderRadius: '8px', background: '#f0fdf4',
                            border: '1px solid rgba(16,185,129,0.12)', fontSize: '0.77rem',
                            color: '#374151', lineHeight: 1.5, maxHeight: '60px', overflow: 'hidden',
                        }}>
                        ...
                        {resultado.datos && resultado.datos.length > 0 && (
                            <div style={{ marginTop: '8px', overflowX: 'auto', borderRadius: '8px', border: '1px solid #e0e0e8' }}>
                                        ...
                                        <tr style={{ background: '#f8f8fc' }}>
                                            {Object.keys(resultado.datos[0]).slice(0, 6).map(col => (
                                                <th key={col} style={{
                                                    ...
                                                    color: '#1a1a2e', borderBottom: '1px solid #e0e0e8',
                                                }}>{col}</th>
                                            ))}
                                            {Object.keys(resultado.datos[0]).length > 6 && (
                                                <th style={{ padding: '6px 8px', color: '#9090a0', borderBottom: '1px solid #e0e0e8' }}>
                                        ...
                                        {resultado.datos.slice(0, 5).map((row, i) => (
                                            <tr key={i}>
                                                {Object.values(row).slice(0, 6).map((val, j) => (
                                                    <td key={j} style={{
                                                        padding: '5px 8px', borderBottom: '1px solid #f0f0f0', color: '#5a5a6e',
                                ...
                                {resultado.datos.length > 5 && (
                                    <p style={{
                                        margin: 0, padding: '6px 8px', fontSize: '0.72rem', color: '#9090a0',
                                        borderTop: '1px solid #e0e0e8', background: '#f8f8fc',
                                    }}>
                ...
                {estado === 'error' && (
                    <div style={{
                        ...
                        <button onClick={resetear} style={{
                            marginTop: '6px', padding: '5px 12px', borderRadius: '6px', border: '1px solid #e0e0e8',
